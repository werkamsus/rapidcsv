...
FORK: https://github.com/werkamsus/rapidcsv

modified to support files that don't begin with a ","
	...
	*/
	...
	public:
		...
		*/
		...
		{
			const size_t columnIdx = pColumnIdx + (mLabelParams.mRowNameIdx);

			while (pColumn.size() + (mLabelParams.mColumnNameIdx) > GetDataRowCount())
			{
			...
			if ((columnIdx) > GetDataColumnCount())
			{
				for (auto itRow = mData.begin(); itRow != mData.end(); ++itRow)
				{
					itRow->resize(columnIdx + (mLabelParams.mRowNameIdx));
				}
			}
			...
			for (auto itRow = pColumn.begin(); itRow != pColumn.end(); ++itRow)
			{
				...
				mData.at(std::distance(pColumn.begin(), itRow) + (mLabelParams.mColumnNameIdx)).at(columnIdx) = str;
			}
		}
		...
		*/
		...
		{
			return (mData.size() > 0) ? (mData.at(0).size() - (mLabelParams.mRowNameIdx)) : 0;
		}
		...
		*/
		...
		{
			const size_t rowIdx = pRowIdx + (mLabelParams.mColumnNameIdx);
			...
			if (pRow.size() > GetDataColumnCount())
			{
				for (auto itRow = mData.begin(); itRow != mData.end(); ++itRow)
				{
					itRow->resize(pRow.size() + (mLabelParams.mRowNameIdx));
				}
			}
			...
			for (auto itCol = pRow.begin(); itCol != pRow.end(); ++itCol)
			{
				...
				mData.at(rowIdx).at(std::distance(pRow.begin(), itCol) + (mLabelParams.mRowNameIdx)) = str;
			}
		}
		...
		*/
		...
		{
			return mData.size() - (mLabelParams.mColumnNameIdx);
		}
	...
	private:
		...
		ssize_t GetRowIdx(const std::string& pRowName) const
		{
			if (mLabelParams.mRowNameIdx >= 0)
			{
				...
				{
					return mRowNames.at(pRowName) - (mLabelParams.mColumnNameIdx);
				}
			}
			return -1;
		}
